...
// ==================================================
// c_n: Territory Control (Mesafe Bazlı Hakimiyet)
// ==================================================
// Bu fonksiyon eski 'countReachable' yerine geçer.
// Her kareye kimin daha hızlı (daha az adımda) ulaştığını hesaplar.
// Duvarlar tam kapanmasa bile yolu uzatmak puan kazandırır.
int calculateTerritory(const State& s) {
    // 1. Mesafe haritalarını "Sonsuz" (999) ile başlat
    int distAI[N][N];
    int distHU[N][N];
    ...
    // 2. BFS Yardımcı Lambda Fonksiyonu
    // (Verilen başlangıç noktasından tüm tahtaya olan en kısa mesafeleri bulur)
    auto runBFS = [&](int startX, int startY, int distMap[N][N]) {
        std::vector<std::pair<int,int>> q; // Kuyruk
        q.push_back({startX, startY});
        
        distMap[startX][startY] = 0; // Başlangıç noktası 0 uzaklıkta
        ...
        while(head < q.size()){
            ...
            for(int k=0; k<8; k++){
                ...
                // Sadece Duvarlar (BLOCKED) engeldir.
                // Territory mantığında piyonların olduğu kareler engel sayılmaz,
                // çünkü amaç "kim o kareye daha önce varabilir" yarışıdır.
                if(inBounds(nx, ny) && s.board[nx][ny] != BLOCKED) {
                    
                    // Daha kısa bir yol bulduysak güncelle ve kuyruğa ekle
    ...
    // 3. İki oyuncu için de BFS çalıştır
    runBFS(s.aiX, s.aiY, distAI); // AI Mesafeleri
    runBFS(s.huX, s.huY, distHU); // Human Mesafeleri

    // 4. Skorlama (Kareleri Paylaşma)
    int score = 0;
    for(int i=0; i<N; i++) {
        for(int j=0; j<N; j++) {
            // Duvar olan kareleri sayma
            if(s.board[i][j] == BLOCKED) continue; 

            // Eğer kareye ikisi de ulaşamıyorsa (tamamen izole) pas geç
            if(distAI[i][j] == 999 && distHU[i][j] == 999) continue;

            // Karşılaştırma: Kim daha az adımda ulaşıyor?
            if (distAI[i][j] < distHU[i][j]) {
                score++; // AI daha hızlı -> BİZİM BÖLGE (+1)
            } 
            else if (distHU[i][j] < distAI[i][j]) {
                score--; // İnsan daha hızlı -> RAKİP BÖLGE (-1)
            }
            // Eşitse (Aynı mesafedeyse) puan yok, tarafsiz bölge.
        }
    }
...
int eval(const State& s) {
    ...
    int c = calculateTerritory(s) * 10; // weight = 5
