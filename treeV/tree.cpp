...
// DUZELTME 1: Skorlar devasa yapildi
...
int eval(const State& s, int depth) {
    ...
    if (hasNoMoves(s)) {
        if (s.isMaxTurn) {
            // AI Kaybetti -> Oyunu uzat (-depth)
            return LOSE_SCORE - depth;
        } else {
            // AI Kazandı -> Hemen kazan (+depth)
            return WIN_SCORE + depth;
        }
    }

    // Oyun devam ediyorsa Heuristic hesapla
    int a = calculateMobility(s) * 5;     // weight = 5
    int b = calculateBarriers(s) * 2;     // weight = 2
    int c = calculateAreaControl(s) * 10; // weight = 10

    // Skorlari 1000 ile carpiyoruz ki WIN_SCORE'a ulasamasinlar
    // Ama yine de belirgin olsunlar.
    return (a + b + c) * 1000;
}
...
int minimax(const State& s, int depth, int alpha, int beta, int parentId) {
    ...
    if (depth == 0 || hasNoMoves(s)) {
        // DUZELTME 3: eval fonksiyonuna depth gönderildi
        int v = eval(s, depth);
    ...
    if (moves.empty()) {
        int v = eval(s, depth); // depth gönderildi
        currentTurnNodes[myId].score = v;
        return v;
    }

    if (s.isMaxTurn) { // MAX
        int best = -2000000000; // WIN_SCORE'dan kucuk olmali
        ...
        int best = 2000000000; // WIN_SCORE'dan buyuk olmali
        for (const auto& m : moves) {
            State child = applyMove(s, m);
            int val = minimax(child, depth - 1, alpha, beta, myId);
